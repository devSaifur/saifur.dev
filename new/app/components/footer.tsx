export function Footer() {
  return (
    <footer className="py-8 text-justify text-sm text-white/80 md:text-center">
      Loosely designed and coded in{' '}
      <span className="font-semibold text-accent/70">VS Code</span> by yours
      truly. Built with{' '}
      <span className="font-semibold text-accent/70">Astro</span> and{' '}
      <span className="font-semibold text-accent/70">Tailwind CSS</span>,
      deployed to{' '}
      <span className="font-semibold text-accent/70">Cloudflare</span>. All text
      is set in the <span className="font-semibold text-accent/70">Geist</span>
      typeface.
    </footer>
  )
}
